...
ENABLE_LATE_PMA="${CMPUNLOCKER_ENABLE_LATE_PMA:-}"
...
if [[ -n "${MCLK_NDIV}" || -n "${MCLK_TIMINGS}" || -n "${ENABLE_P2P}" || -n "${ENABLE_LATE_PMA}" ]]; then
    {
        ...
        [[ -n "${ENABLE_LATE_PMA}" ]] && echo "#define CMPUNLOCK_ENABLE_LATE_PMA 1"
        echo "#endif"
    ...
    if [[ -n "${ENABLE_LATE_PMA}" ]]; then
        ok "Late PMA extension enabled (publishes high reserved FB regions)"
    else
        info "Late PMA extension compiled out; WPR/GSP regions stay reserved"
    fi
else
